...
If a type/action/API field is missing → message Soroush. Do not patch his logic.
...
- Success: `{ components, totalBudget }` → build `WeddingPlan` with `generatedAt` + `formData`, then `setPlan` + `setView('planner')`
...
1. Wait for Soroush scaffold push → `npm install`
...
5. **Landing**: "TBD" + "Your wedding, thoughtfully planned." + Start Planning → `setView('form')`
...
8. **PlannerView**: header summary (location · date · guests · vibe), Start Over → `clearPlan`, sticky BudgetBar, card grid, Add Component modal
